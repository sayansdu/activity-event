...
public class EventService {
     ...
     * @throws IOException 
     */
    public void saveEvent(Message msg) throws JMSException, IOException {
        Util util = new Util();
        ...
        if(deserialize.getEventType()==null || !(deserialize.getEventType() instanceof Long)){
        	...
        	event.setEventType( dao.getEventTypeByCode(deserialize.getEventType()) );
        ...
        else if(dao.getUserByUserId((long) deserialize.getUser())==null ){
        	System.out.println("User don't exist, event can't be saved");
        	return;
        }
        else
        	event.setUser( dao.getUserByUserId((long) deserialize.getUser()) );
